...
class Paper extends Base{
	...
	public function getReportList(){
		...
		if($count < 10){
			$nextStartId = -1;
			$data = $report->where($where)->field($field)->order('create_time desc')->select();
		}else{
			$nextStartId = $param['nextStartId'];
			$data = $report->where($where)->field($field)->limit($nextStartId,10)->order('create_time desc')->select();
		...
		{
			$data[$key]['detail_url'] = config('view_replace_str.__ADMROOT__') . "index/Spa/getSpa#/newsdetail/id/" . $val['id'] . "/uid/" . $param['userId'] . "/type/3";
