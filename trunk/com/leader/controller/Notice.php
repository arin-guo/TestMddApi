...
class Notice extends Base{
	...
	public function getMailtelList(){
		$Teacher = model('Teachers');$Leader = model('Headmasters');$Mailtel = model('Mailtel');
		$param = $this->param;
		$info = $Leader->where('id',$param['userId'])->find();
		$field = '0 as id,photo,realname as name,tel,"" as identity,"" as remark';
		$data = $Teacher->where('flag',1)->where('school_id',$info['school_id'])->where('is_job',1)->field($field)->select();
		//获取自定义的电话号码
		$pdata = $Mailtel->where('flag',1)->where('school_id',$info['school_id'])->where('user_id',$param['userId'])->where('type',3)->field('id,photo,name,tel,identity,remark')->select();
		$i = count($data);
